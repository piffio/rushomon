...
export const analyticsApi = {
	...
	async getOrgAnalytics(days: number): Promise<OrgAnalyticsResponse> {
		return apiClient.get<OrgAnalyticsResponse>(`/api/analytics/org?days=${days}`);
	},
	 ...
	 */
	async getOrgAnalyticsCustomRange(start: number, end: number): Promise<OrgAnalyticsResponse> {
		return apiClient.get<OrgAnalyticsResponse>(`/api/analytics/org?start=${start}&end=${end}`);
	}
};
