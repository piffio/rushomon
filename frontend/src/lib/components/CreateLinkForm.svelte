<script lang="ts">
  ...
  async function handleSubmit(e: Event) {
    ...
    if (trimmedShortCode) {
      ...
      if (trimmedShortCode.startsWith("-") || trimmedShortCode.endsWith("-")) {
      ...
      if (trimmedShortCode.includes("//")) {
        error = "Custom code cannot contain consecutive forward slashes";
        isSubmitting = false;
        return;
      }
      ...
      if (segments.length > 3) {
        error = "Custom code can have at most 3 segments separated by slashes";
        isSubmitting = false;
        return;
      }
    ...
    try {
      const link = await linksApi.create({
        ...
        forward_query_params: isPro && forwardQueryParams ? true : undefined,
  ...
  <form onsubmit={handleSubmit} class="space-y-4">
    ...
    <div>
      ...
      >
        ...
        <span class="text-gray-500 text-xs font-normal"
          >(Optional, 3-100 characters: letters, numbers, hyphens, forward
          slashes)</span
        >
      </label>
      <input
        ...
        {#if showUtmBuilder}
          <div class="p-4 space-y-3 border-t border-gray-200">
            <p class="text-xs text-gray-500">
              These UTM parameters will be appended to the destination URL on
              every redirect.
            </p>
      ...
      <div class="flex items-start gap-3 p-4 border border-gray-200 rounded-lg">
        <div class="flex-1">
          ...
          <p class="text-xs text-gray-500 mt-0.5">
            Append any query params from the short link URL to the destination.
            Visitor params override UTM params on conflict.
          </p>
        </div>
        <input
          type="checkbox"
    ...
    {:else}
      ...
      >
        ...
        <span
          ><strong class="text-gray-700">Pro feature:</strong> UTM parameters
          &amp; query forwarding —
