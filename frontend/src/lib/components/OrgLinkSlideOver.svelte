<script lang="ts">
	import type { TopLinkCount, LinkAnalyticsResponse, UserAgentCount } from '$lib/types/api';
	import { linksApi } from '$lib/api/links';
	import { PUBLIC_VITE_SHORT_LINK_BASE_URL, PUBLIC_VITE_API_BASE_URL } from '$env/static/public';
	...
	const SHORT_LINK_BASE =
		PUBLIC_VITE_SHORT_LINK_BASE_URL || PUBLIC_VITE_API_BASE_URL || 'http://localhost:8787';
	...
	const chartColors = ['#f97316','#3b82f6','#10b981','#8b5cf6','#ef4444','#f59e0b','#06b6d4','#ec4899'];
		...
		{:else if analytics}
			...
			{#if analytics.clicks_over_time.length > 0}
				<div class="bg-white border border-gray-200 rounded-xl p-4">
					<h3 class="text-xs font-semibold text-gray-600 mb-3">Clicks over time</h3>
					<div class="h-36">
			...
			{#if analytics.top_referrers.length > 0}
				<div class="bg-white border border-gray-200 rounded-xl p-4">
					<h3 class="text-xs font-semibold text-gray-600 mb-3">Top referrers</h3>
							...
							<div class="flex items-center justify-between gap-3 text-sm">
								...
								<div class="flex items-center gap-2 flex-shrink-0">
									<div class="w-12 h-1.5 bg-gray-100 rounded-full overflow-hidden">
										<div class="h-full bg-blue-400 rounded-full" style="width: {barWidth(ref.count, analytics.top_referrers)}%"></div>
									</div>
									<span class="text-gray-600 font-medium w-8 text-right">{ref.count}</span>
								</div>
							</div>
						{/each}
					</div>
			...
			{#if analytics.top_countries.length > 0}
				<div class="bg-white border border-gray-200 rounded-xl p-4">
					<h3 class="text-xs font-semibold text-gray-600 mb-3">Top countries</h3>
					<div class="space-y-1.5">
						{#each analytics.top_countries.slice(0, 5) as country}
							<div class="flex items-center justify-between gap-3 text-sm">
								...
								<div class="flex items-center gap-2 flex-shrink-0">
									<div class="w-12 h-1.5 bg-gray-100 rounded-full overflow-hidden">
										<div class="h-full bg-green-400 rounded-full" style="width: {barWidth(country.count, analytics.top_countries)}%"></div>
									</div>
									<span class="text-gray-600 font-medium w-8 text-right">{country.count}</span>
								</div>
							</div>
						{/each}
					</div>
