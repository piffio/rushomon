<script lang="ts">
	...
	function getKeyStatus(
		...
		if (key.expires_at && key.expires_at < Date.now() / 1000)
			return "expired";
		return "active";
	}
...
<div class="page">
	<div class="page-header">
		<h1>API Keys</h1>
		<p class="subtitle">
			View and manage all API keys across the instance. Revoke keys in
			case of abuse.
		</p>
	</div>
				...
				<tbody>
					{#each keys as key (key.id)}
						...
						<tr class:row-revoked={status === "revoked"}>
							...
							<td class="date-cell"
								>{formatDate(key.created_at)}</td
							>
							<td class="date-cell"
								>{formatDate(key.last_used_at)}</td
							>
							<td class="date-cell"
								>{formatDate(key.expires_at)}</td
							>
							<td>
								{#if status === "deleted"}
		...
		<div class="modal">
			...
			<p>
				{#if confirmingAction === "revoke"}
					...
					>? This will immediately invalidate the key and prevent it
					from being used.
				{:else if confirmingAction === "reactivate"}
				...
				{:else if confirmingAction === "delete"}
					Are you sure you want to delete <strong
						>{confirmingKeyName}</strong
					>? This will soft-delete the key and hide it from normal
					views.
				{:else if confirmingAction === "restore"}
