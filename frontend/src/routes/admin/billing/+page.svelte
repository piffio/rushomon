    ...
    const promises = accounts.map(async (account) => {
      if (!accountDetails[account.id]) {
        try {
          ...
          console.error(
            `Failed to load details for account ${account.id}:`,
            err
          );
        }
      }
    ...
    <div class="accounts-list">
      {#each accounts as account (account.id)}
        <div
          ...
          class="account-card {expandedId === account.id ? 'expanded' : ''}"
        >
          <!-- Collapsed Header -->
          <button class="card-header" onclick={() => toggleExpand(account.id)}>
            <span class="owner">{account.owner_email}</span>
            <span class="tier-badge {getTierBadgeClass(account.tier)}"
            ...
            {#if accountDetails[account.id]?.subscription}
              ...
              {@const daysUntilEnd = sub?.current_period_end
                ? Math.floor(
              ...
              {#if sub?.pending_cancellation}
                <span
                  class="subscription-status-badge pending {daysUntilEnd !==
                    null && daysUntilEnd <= 7
                    ? 'urgent'
                    : ''}"
                  title="Cancels {sub.current_period_end
              ...
              {/if}
            {/if}
            <span class="usage">{account.links_created_this_month} links</span>
            <span class="orgs">{account.org_count} org(s)</span>
            <span class="icon">{expandedId === account.id ? "▼" : "▶"}</span>
          </button>
          ...
          {#if expandedId === account.id}
            <div class="card-body">
              ...
              {:else if accountDetails[account.id]}
                ...
                <div class="section">
                  ...
                  <div class="info-grid">
                    <div class="info-item">
                      ...
                      <span class="value"
                        >{details.owner.email} ({details.owner.name ||
                          "No name"})</span
                      >
                    </div>
                    <div class="info-item">
                      ...
                      <button
                        ...
                        onclick={() =>
                          handleTierChange(account.id, details.account.tier)}
                        disabled={tierLoading}
                      >
                        {getTierDisplayName(details.account.tier)} ▼
                      </button>
                    </div>
                    <div class="info-item">
                      <span class="label">Created:</span>
                      <span class="value"
                        >{formatDate(details.account.created_at)}</span
                      >
                    </div>
                  </div>
                </div>
                ...
                <div class="section">
                  <h4>
                    Usage This Month ({details.usage.year_month})
                  </h4>
                  <div class="usage-bar">
                    <div class="usage-text">
                      {details.usage.links_created_this_month} /
                      {details.usage.max_links_per_month ?? "∞"} links
                    </div>
                    {#if details.usage.max_links_per_month}
                      {@const percentage =
                        (details.usage.links_created_this_month /
                          details.usage.max_links_per_month) *
                        100}
                ...
                <div class="section">
                    ...
                    <div class="subscription-info">
                      <div class="info-grid">
                        <div class="info-item">
                          ...
                          {#if sub.pending_cancellation}
                            <span
                              class="value subscription-status pending {daysUntilEnd !==
                                null && daysUntilEnd <= 7
                                ? 'urgent'
                                : ''}"
                            >
                              ...
                              {:else}
                                🟡 Cancels {sub.current_period_end
                                  ? formatDate(sub.current_period_end)
                                  : ""}
                              {/if}
                            </span>
                          {:else if sub.status === "active"}
                            <span class="value subscription-status active">
                              🟢 Active
                            </span>
                          {:else}
                            <span class="value subscription-status canceled">
                              🔴 Canceled
                            </span>
                          {/if}
                        </div>
                        ...
                        <div class="info-item">
                          ...
                          <span class="value">{sub.interval}</span>
                        </div>
                        {#if sub.amount_cents}
                          <div class="info-item">
                            <span class="label">Price:</span>
                            <span class="value">
                              €{(sub.amount_cents / 100).toFixed(
                                2
                        ...
                        {#if sub.current_period_start}
                          <div class="info-item">
                            ...
                            <span class="value"
                              >{formatDate(sub.current_period_start)}</span
                            >
                          </div>
                        {/if}
                      ...
                      <div class="subscription-actions">
                        {#if sub.status === "active"}
                          <button
                            ...
                            onclick={() =>
                              handleSubscriptionUpdate(account.id, "canceled")}
                        ...
                        {:else if sub.status === "canceled"}
                          <button
                            ...
                            onclick={() =>
                              handleSubscriptionUpdate(account.id, "active")}
                            disabled={subscriptionLoading}
                          >
                  ...
                  {:else}
                    <p class="no-subscription">No subscription found</p>
                  {/if}
                </div>
                    ...
                    <div class="orgs-list">
                      {#each details.organizations as org}
                        <div class="org-card">
                          ...
                          <div class="org-stats">
                            <span>{org.link_count} links</span>
                ...
                <div class="section actions-section">
                  ...
                  <button
                    class="btn-danger"
                    onclick={() => handleResetCounter(account.id)}
                    disabled={loading}
                  >
  ...
  >
    ...
    >
      <div class="modal-header">
        ...
        <button class="modal-close" onclick={cancelTierChange}>&times;</button>
      </div>
      <div class="modal-body">
        <p>
          Select new tier for this billing account. This will affect all
...
{#if confirmingReset}
  ...
  >
    ...
    >
      <div class="modal-header">
        <h3>Reset Counter?</h3>
        <button class="modal-close" onclick={cancelReset}>&times;</button>
      </div>
      <div class="modal-body">
        <p>
          Are you sure you want to <strong>reset the monthly counter</strong> for
          this billing account? This will set the links created count to 0 for the
          current month.
        </p>
        <p class="warning">
    ...
    >
      ...
      <div class="modal-body">
        ...
        {#if confirmingSubscriptionUpdate.currentStatus === "canceled"}
          <p class="warning">
            This will mark the subscription as canceled and update the billing
            account to free tier.
          </p>
        {:else}
          <p class="info">This will mark the subscription as active again.</p>
        {/if}
      </div>
      <div class="modal-footer">
        <button
        ...
        <button
          class="btn {confirmingSubscriptionUpdate.currentStatus === 'canceled'
            ? 'btn-warning'
            : 'btn-primary'}"
