<script lang="ts">
  ...
  async function confirmBlockDomain() {
    ...
    try {
      ...
      await adminApi.blockDestination(domain, "domain", "Reviewed via report");
      // Auto-resolve the report
      await adminApi.updateReportStatus(
        selectedReport.id,
        "reviewed",
...
<div class="reports-page">
  <div class="container mx-auto px-4 py-8">
    ...
    {:else if currentUser && currentUser.role !== "admin"}
      ...
      <div class="text-center py-8 bg-white rounded-lg shadow-md">
        <div class="mb-4">
          <div class="text-red-600 text-xl mb-2">⛔ Access Denied</div>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm">
                      ...
                      <div
                        ...
                        title={report.link.destination_url}
                      >
                    ...
                    {#if report.status === "pending"}
                      <div class="flex flex-wrap gap-2">
                        <button
                          ...
                          onclick={() => handleBlockDestinationURL(report)}
                          title="Block this exact URL">Block URL</button
                        >
                        <button
                          class="px-3 py-1 bg-purple-600 text-white text-xs rounded hover:bg-purple-700 transition-colors"
                          onclick={() => handleBlockDestinationDomain(report)}
                          title="Block this entire domain">Block Domain</button
                        >
                        <button
                          class="px-3 py-1 bg-orange-600 text-white text-xs rounded hover:bg-orange-700 transition-colors"
                          onclick={() => handleDisableLink(report)}
                          title="Disable this link only">Disable</button
                        >
                        <button
                          class="px-3 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700 transition-colors"
                          onclick={() => handleDismissReport(report)}
                          title="Dismiss report">Dismiss</button
                        >
                      </div>
                    {:else}
  ...
  >
    ...
    >
      ...
      <div class="modal-footer">
        ...
        <button class="btn-danger" onclick={confirmBlockUrl}>Block URL</button>
      </div>
    </div>
  </div>
{/if}
