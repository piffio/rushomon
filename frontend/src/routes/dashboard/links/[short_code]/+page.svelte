<script lang="ts">
  ...
  function createBrowserChart() {
    ...
    browserChart = new Chart(browserChartCanvas, {
      type: "doughnut",
      data: {
        labels: browserData.map((b, i) => `${b.name} (${percentages[i]}%)`),
        datasets: [
          {
            data: browserData.map((b) => b.count),
            backgroundColor: chartColors.slice(0, browserData.length),
            borderWidth: 2,
            borderColor: "#fff"
          }
        ]
  ...
  function createOSChart() {
    ...
    const percentages = osData.map((o) => ((o.count / total) * 100).toFixed(1));
      ...
      <div class="bg-white border-2 border-gray-200 rounded-xl p-6 mb-6">
        ...
        >
          <div class="flex-1 min-w-0">
            <h1 class="text-2xl font-bold text-gray-900 truncate mb-2">
          ...
          <!-- Summary Stats -->
          <div class="flex items-center gap-6 text-center sm:flex-shrink-0">
            <div>
          ...
          <div class="relative">
            ...
            {#if lockedPopoverOpen === range.label}
              ...
              >
                ...
                >
                  Upgrade to {range.minTier === "business" ? "Business" : "Pro"} →
                </a>
                <button
          ...
          >
            {#if analytics.analytics_gated && analytics.gated_reason === "retention_limited"}
              ...
              <p class="text-center mb-3">
                This time range exceeds your free tier analytics retention.
              </p>
              <a
                href="/pricing"
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between mb-1">
                      <span class="text-sm text-gray-900"
                        >{countryCodeToName(country.country)}</span
                      >
                      <span class="text-sm font-medium text-gray-600 ml-2"
                        >{country.count}</span
                      >
