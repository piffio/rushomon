					...
					<ol class="flex items-center gap-0">
						{#each STEPS as label, i}
							...
							>
								...
								{#if i < STEPS.length - 1}
									<div
										class="flex-1 h-px bg-gray-200 mx-2"
									></div>
								{/if}
							</li>
						{/each}
	...
	<main class="max-w-6xl mx-auto px-6 py-8">
		...
		{#if step === 1}
			<div class="bg-white rounded-2xl border border-gray-200 p-8">
				...
				<p class="text-sm text-gray-500 mb-6">
					Your CSV must have a header row. We'll auto-detect common
					column names.
				</p>
				...
				>
					...
					<p class="text-xs text-gray-400 mt-1">
						Supports .csv files up to any size (batched in chunks of
						50)
					</p>
				</label>
				...
				<div
					class="mt-6 p-4 bg-blue-50 rounded-xl text-sm text-blue-700"
				>
					<strong>Tip:</strong> Your CSV must include at least a
					destination URL column. Optional columns: short code (Pro+;
					full URLs like https://dub.co/abc are auto‑extracted to
					"abc", supports letters, numbers, hyphens, and forward
					slashes), title, tags (comma or pipe-separated:
		...
		{#if step === 2}
			<div class="bg-white rounded-2xl border border-gray-200 p-8">
				...
				<p class="text-sm text-gray-500 mb-6">
					{rawRows.length} rows detected. Map your CSV columns to the fields
					below.
				</p>
		...
		{#if step === 3}
			...
			<div class="bg-white rounded-2xl border border-gray-200 p-8">
				...
				<p class="text-sm text-gray-500 mb-6">
					...
					rows.
					{#if validationErrors.size > 0}{validationErrors.size} row(s)
						have issues.{:else}All previewed rows look valid.{/if}
				</p>
					...
					<table class="w-full text-sm">
						...
						<tbody>
							{#each preview as row, i}
								...
								<tr
									class="border-t border-gray-100 {hasError
										? 'bg-red-50'
										: ''}"
								>
									...
									{#if columnMap.title}
										<td
											class="px-3 py-2 text-gray-600 truncate max-w-[150px]"
											>{row.title || "—"}</td
										>
									{/if}
									<td class="px-3 py-2">
				...
				<div class="space-y-4 mt-6">
					...
					<div class="bg-gray-50 rounded-xl p-4 text-sm space-y-1">
						...
						{#if remainingQuota !== null}
							<div class="flex justify-between">
								<span class="text-gray-500"
									>Remaining monthly quota</span
								>
								<span
									class="font-semibold {validRows.length >
									remainingQuota
										? 'text-red-600'
										: 'text-green-600'}"
								>
					...
					{#if !isProOrAbove && columnMap.short_code}
						...
						>
							<strong>Free tier:</strong> Custom short codes are
							not available on the Free plan. All links will be
							assigned auto-generated short codes.
			...
			<div class="bg-white rounded-2xl border border-gray-200 p-8">
				...
				{:else}
					...
					{#if allWarnings.length > 0}
						<div class="mb-4">
							<h3
								class="text-sm font-semibold text-amber-700 mb-2"
							>
							...
							>
								{#each allWarnings as warn}
									<div class="px-3 py-2 text-xs flex gap-2">
										...
										>
										<span
											class="font-mono text-gray-600 truncate"
						...
						<div class="mb-6">
							...
							>
								...
								{#if allErrors.length > 50}
									...
									>
										…and {allErrors.length - 50} more. Download
