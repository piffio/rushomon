<script lang="ts">
	...
	function aggregateByKey(
		...
		for (const p of parsed)
			map.set(p[key], (map.get(p[key]) || 0) + p.count);
	...
	const avgClicksPerLink = $derived(() => {
		...
		return Math.round(
			analytics.total_clicks / analytics.unique_links_clicked,
		);
	});
	...
	function buildFilteredParams(): URLSearchParams {
		...
		if (activeCountries.length === 1)
			params.set("country", activeCountries[0]);
			...
			{#if analytics}
					...
					>
						<div>
							...
							<p class="text-sm text-orange-700 mt-0.5">
								Upgrade to Pro to unlock up to 1 year of
								analytics history, custom date ranges, and more.
							</p>
						</div>
						<a
							href="/pricing"
							...
							<div class="space-y-1">
								{#each analytics.top_links as link, i}
									...
									>
										...
										>
											<span
												class="text-xs text-gray-400 w-4 flex-shrink-0"
												>{i + 1}</span
											>
											<div class="min-w-0">
												<span
												...
												{#if link.title}
													<span
														class="text-xs text-gray-400 block truncate"
														>{link.title}</span
													>
												{/if}
											</div>
										...
										>
											...
											<span
												class="text-gray-700 font-medium w-12 text-right"
				...
				<div class="bg-white rounded-xl border border-gray-200 p-5">
					...
					{#if analytics.top_countries.length > 0}
						{@const maxCount =
							analytics.top_countries[0]?.count ?? 1}
						<div class="space-y-1">
							{#each analytics.top_countries as country}
								...
								>
									<div
										class="flex items-center gap-2 text-gray-700 truncate"
									>
