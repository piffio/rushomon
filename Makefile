...
version-bump-patch:
	...
	@$(MAKE) version-sync
	@echo "✅ Version bumped to $(shell grep -A2 '^\[package\]' Cargo.toml | grep '^version = ' | sed 's/version = "//' | sed 's/"//')"
...
release: version-bump-patch version-tag
	@echo "🚀 Release v$(shell grep -A2 '^\[package\]' Cargo.toml | grep '^version = ' | sed 's/version = "//' | sed 's/"//') ready!"
