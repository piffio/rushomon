...
generate_wrangler_config() {
  ...
  if [ -z "$R2_ASSETS_BUCKET_NAME" ]; then
    error "R2_ASSETS_BUCKET_NAME is not set. Set it to the name of your assets R2 bucket."
    return 1
  fi
...
deploy_environment() {
  ...
  # Create R2 assets bucket (idempotent)
  if ! create_or_get_r2_bucket "${R2_ASSETS_BUCKET_NAME}"; then
    error "Failed to create/get R2 assets bucket"
    return 1
  fi
...
show_deployment_summary() {
  ...
  echo "  - R2 Bucket:      ${R2_ASSETS_BUCKET_NAME}"
  echo ""
