...
OUTPUT_DIR="${PROJECT_DIR}/docs/openapi"
mkdir -p "$OUTPUT_DIR"
...
./target/debug/generate_openapi > "$OUTPUT_DIR/main.json" 2>/dev/null

if [ $? -eq 0 ]; then
    echo "✅ OpenAPI spec generated successfully!"
    echo "📁 Output: $OUTPUT_DIR/main.json"

    # Show some stats
    LINES=$(wc -l < "$OUTPUT_DIR/main.json")
    SIZE=$(du -h "$OUTPUT_DIR/main.json" | cut -f1)
